...
local function IsInsideOwnedProperty()
    return LocalPlayer.state.insideProperty ~= nil
        and LocalPlayer.state.insideProperty ~= false
end
...
local houseLoot = {}
local house = 1
local insideRobberyHouse = false
...
local function handleHouseExits(interiorId)
    if not insideRobberyHouse then return end
...
RegisterNetEvent('qbx_houserobbery:client:enterHouse', function()
    insideRobberyHouse = true
    setupLoot()
end)
...
AddStateBagChangeHandler("insideProperty", nil, function(bagName, key, value)
    if bagName ~= ("player:%s"):format(GetPlayerServerId(PlayerId())) then return end
    if not value then return end

    -- Cancela qualquer sessão visual do roubo ao entrar pelo propertys.
    if lib and lib.hideTextUI then
        lib.hideTextUI()
    end

    TriggerEvent("qbx_houserobbery:client:forceCleanup")
end)

RegisterNetEvent("qbx_houserobbery:client:forceCleanup", function()
    -- Variáveis mais comuns do recurso; só altera as que existirem.
    if currentHouse ~= nil then currentHouse = nil end
    if houseIndex ~= nil then houseIndex = nil end
    if insideHouse ~= nil then insideHouse = false end
    if inHouse ~= nil then inHouse = false end
end)
