...
local function enterHouse(source, coords, bucket, closestHouseIndex)
    ...
    if not closestHouseIndex or not sharedConfig.houses[closestHouseIndex] then return end

    SetResourceKvpInt(('houserobbery:%s'):format(player.PlayerData.citizenid), closestHouseIndex)
...
AddEventHandler('lockpicks:UseLockpick', function(playerSource, isAdvanced)
    ...
    -- O propertys controla interior, loot e cooldown de suas casas dinâmicas.
    -- Ele recebe a lockpick antes da tabela estática deste recurso.
    if GetResourceState('propertys') == 'started' then
        local propertyName = exports.propertys:FindPropertyAtCoords(playerCoords, 3.0)
        if propertyName then
            TriggerClientEvent('propertys:client:startRobberyFromQbx', playerSource, propertyName)
            return
        end
    end
...
local function IsPlayerInsideProperty(src)
    local player = Player(src)
    return player and player.state and player.state.insideProperty
end

local function GetValidHouse(index)
    index = tonumber(index)
    if not index then return nil, nil end

    local house = sharedConfig and sharedConfig.houses and sharedConfig.houses[index]
    if not house then return nil, index end

    return house, index
end

RegisterNetEvent('qbx_houserobbery:server:enterHouse', function(index)
    if IsPlayerInsideProperty(source) then return end
    local house, validIndex = GetValidHouse(index)
    if not house then
        print(("[qbx_houserobbery] Índice de casa inválido recebido de %s: %s"):format(source, tostring(index)))
        return
    end
    index = validIndex
...
RegisterNetEvent('qbx_houserobbery:server:leaveHouse', function()
    ...
    local kvpKey = ('houserobbery:%s'):format(player.PlayerData.citizenid)
    local index = GetResourceKvpInt(kvpKey)

    -- O jogador pode estar em um interior residencial compartilhado sem ter
    -- entrado por este recurso. Nesse caso não existe uma casa de roubo ativa.
    ...
    if not interior or not interior.exit then
        DeleteResourceKvp(kvpKey)
        return
    end
    ...
    local exit = interior.exit.xyz or vec3(interior.exit.x, interior.exit.y, interior.exit.z)

    if #(playerCoords - exit) > 3.0 then return end
